...
while prox_it
    ...
    for i = 1:c % Calcula as diatancias dos dados aos prototipos
        for k = 1:N
            D(i,k) = (Z(:,k)-VV{l-1}(:,i))'*A*(Z(:,k)-VV{l-1}(:,i));
        end
    end
    mu{l,1} = zeros(c,N); % Inicializa a matriz de particao atual
    for k = 1:N
        % Armazena os indices de elementos nulos em D(:,k)
        indcn = find(D(:,k)==0);
        if isempty(indcn) % Se nao houver elementos nulos
            for i = 1:c
                for j = 1:c
                    mu{l}(i,k) = mu{l}(i,k) + (D(i,k)/D(j,k))^(1/(m-1));
                end
                mu{l}(i,k) = 1/mu{l}(i,k);
            end
        else % Se houver elementos nulos em D(:,k)
    ...
    for i = 1:c
        num = 0; den = 0;
        for k = 1:N
            num = num + mu{l}(i,k)^m*D(i,k);
            den = den + mu{l}(i,k)^m;
        end
        S2(i) = lambda*num/den;
    end
    f = zeros(c,N); fl = zeros(c,N);
    for i = 1:c
        for k = 1:N
            f(i,k) = 1 - exp(-D(i,k)/S2(i));
            fl(i,k) = 1/S2(i)*exp(-D(i,k)/S2(i));
        end
    end
    ...
    for k = 1:N
        ...
        if isempty(indcn) % Se nao houver elementos nulos
            for i = 1:c
                for j = 1:c
                    UU{l}(i,k) = UU{l}(i,k) + (f(i,k)/f(j,k))^(1/(m-1));
                end
                UU{l}(i,k) = 1/UU{l}(i,k);
            end
        else % Se houver elementos nulos em D(:,k)
    ...
    for i = 1:c % Calcula a matriz de prototipos atual
        num = zeros(n,1); den = 0; % Para calcular o somatorio
        for k = 1:N % Efetua os somatorios
            num = num + UU{l}(i,k)^m*fl(i,k)*Z(:,k);
            den = den + UU{l}(i,k)^m*fl(i,k);
        end
...
hold on
plot(u_,y_,'k');
centros_nrFCM = VV{end};
save('centros_nrFCM.mat','centros_nrFCM');